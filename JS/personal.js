//this functions open and close the contac form
function openForm(){
...
function showSlides(n){
    ...
    for (var i=0;i<slides.length;i++){
        slides[i].style.display="none";//this for loop takes each item in the array "slides" and set the display to none
    }
    for (var i=0;i<slides.length;i++){
    ...
    slides[slideIndex-1].style.display="block";//tis displays the image in the slideshow
    dots[slideIndex-1].className +=" active";//this adds the active styling in the dot associated with the image
}

//this code will create vlose the contact form when the user clicks off of it
//the first step is to add an event listener for any clicks on the website
document.addEventListener("click", function(event){
    //here we state that if the click happens on the cancel button OR anywhere that is not the contact form AND the click does not happenon any element woth the content class then call the closeFrom() function
